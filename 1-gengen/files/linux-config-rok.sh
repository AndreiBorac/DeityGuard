...
function doit()
{
  (
    ...
    _ MMU=y MULTIPLATFORM=y VIRT=y ROCKCHIP=y ARM_THUMB{,EE}=y KUSER_HELPERS=y VDSO=y ARM_KERNMEM_PERMS=y DEBUG_RODATA=y
    # errata go here
    # end errata
    _ ARCH_MULTI_V7=y ARCH_ROCKCHIP=y
    if [ "$ALSO_QEMU" == "y" ] || true # actually, disabling this causes problems on the real hardware. go figure.
    then
      _ ARCH_VEXPRESS=y
    fi
    _ SMP=y SMP_ON_UP=y ARM_CPU_TOPOLOGY=y VMSPLIT_2G=y NR_CPUS=16 HOTPLUG_CPU=y PREEMPT=y HZ_1000=y ARM_PATCH_IDIV=y AEABI=y HIGHMEM=y HIGHPTE=y CPU_SW_DOMAIN_PAN=y HW_PERF_EVENTS=y COMPACTION=y BOUNCE=y FORCE_MAX_ZONEORDER=12 SECCOMP=y
    ...
    _ IOMMU_SUPPORT=y ROCKCHIP_IOMMU=y IOMMU_IO_PAGETABLE_{LPAE,ARMV7S}=y
    _ ROCKCHIP_PM_DOMAINS=y
    _ PM_DEVFREQ=y
    ...
    # modules (keyboard/display)
    _ INPUT_EVDEV=m INPUT_KEYBOARD=y KEYBOARD_CROS_EC=m
    _ I2C_{CHARDEV,ALGOBIT,RK3X}=m
    _ SPI=y SPI_ROCKCHIP=m
    _ MFD_CROS_EC{,_SPI}=m
    _ PWM=y PWM_ROCKCHIP=m
    _ DRM=m DRM_FBDEV_EMULATION=y
    (
      # at some point FRAMEBUFFER_CONSOLE changed from tristate to
      # bool; let's detect that so we can remain flexible with kernel
      # version
      X="$(find . -type f | egrep '/Kconfig$' | xargs egrep -A 1 '^config FRAMEBUFFER_CONSOLE$' | wc -l)"
      [ "$X" == "2" ]
      X="$(find . -type f | egrep '/Kconfig$' | xargs egrep -A 1 '^config FRAMEBUFFER_CONSOLE$' | tail -n 1 | ( egrep '\btristate\b' || true ) | wc -l)"
      if [ "$X" == "1" ]
      then
        _ FRAMEBUFFER_CONSOLE=m
      else
        _ FRAMEBUFFER_CONSOLE=y
      fi
    )
    _ DRM_ROCKCHIP=m ROCKCHIP_ANALOGIX_DP=y PHY_ROCKCHIP_DP=m ROCKCHIP_DW_HDMI=y
    _ BACKLIGHT_CLASS_DEVICE=m BACKLIGHT_PWM=m
    _ DRM_PANEL_SIMPLE=m
    _ FB_ARMCLCD=m
    
    # modules (usb)
    _ USB_SUPPORT=y USB=m USB_DEFAULT_PERSIST=y USB_EHCI_HCD=m USB_EHCI_TT_NEWSCHED=y PHY_ROCKCHIP_USB=m USB_DWC2=m
    ...
    # modules (touchpad)
    _ INPUT_MOUSE=y MOUSE_ELAN_I2C=m MOUSE_ELAN_I2C_I2C=y I2C_HID=m
    _ MFD_RK808=m REGULATOR_RK808=m
    
    # modules (usb hid)
    _ USB_HID=m USB_HIDDEV=y
    
    # modules (emmc)
    _ MMC=m MMC_BLOCK=m MMC_BLOCK_BOUNCE=y MMC_DW=m MMC_DW_ROCKCHIP=m PHY_ROCKCHIP_EMMC=m PWRSEQ_EMMC=m #MMC_DEBUG=y MMC_ARMMMCI=m MMC_SDHCI=m PWRSEQ_SIMPLE=m 
